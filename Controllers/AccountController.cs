...
namespace CRM.Controllers
{
    public class AccountController : Controller
    {
        ...
        public AccountController(AppDbContext db, IConfiguration config, IHttpContextAccessor httpContextAccessor)
        {
            _db = db;
            _config = config;
        ...
        {
            // Get all role names as a list of strings
        ...
        {
            // client-side ensured password rules & matching
            var user = await _db.Users.FirstOrDefaultAsync(u => u.Username == model.Username && u.Password == model.oldPassword);
            if (user == null)
            {
                ViewBag.Message = "old Password not matching";
                return View(model);
            }

            user.Password = model.NewPassword; // plain text per request
            user.ResetToken = null;
            ...
            if (userSettings != null)
            {
                userSettings.PasswordLastChanged = DateTime.Now; // or DateTime.UtcNow
                _db.UserSettings.Update(userSettings);
            }
            ...
            TempData["PasswordResetSuccess"] = "Password updated. Please login.";
            return RedirectToAction(nameof(Login));
        }
