...
namespace CRM.Controllers
{
    ...
    
    {
        ...
        public async Task<IActionResult> UploadLeadFile(IFormFile file, int a)
        {
            try
            {
                Console.WriteLine($"DEBUG: UploadLeadFile called by user");
                
                if (file == null || file.Length == 0)
                {
                    Console.WriteLine($"DEBUG: No file provided");
                    TempData["Error"] = "Please select a file to upload";
                    return RedirectToAction("Index");
                }
                
                Console.WriteLine($"DEBUG: File received - Name: {file.FileName}, Size: {file.Length} bytes");
                
                // Check file size (limit to 10MB)
                if (file.Length > 10 * 1024 * 1024)
                {
                    Console.WriteLine($"DEBUG: File too large: {file.Length} bytes");
                ...
                {
                    Console.WriteLine($"DEBUG: Invalid file type: {fileExtension}");
                    TempData["Error"] = "Please upload only Excel files (.xlsx or .xls)";
                    return RedirectToAction("Index");
                }

                var (UserId, Role) = GetUserFromToken();
                Console.WriteLine($"DEBUG: User {UserId} with role {Role} attempting bulk upload");
                
                var currentUser = _db.Users.FirstOrDefault(u => u.UserId == UserId);
                var uploaderName = currentUser?.Username ?? "System";
                Console.WriteLine($"DEBUG: Current user: {uploaderName}, ChannelPartnerId: {currentUser?.ChannelPartnerId}");
            ...
            for (int r = 2; r <= lastRow; r++) // Skip header
            {
                ...
                // Check subscription limits for partners before processing each lead
                if (currentUser?.ChannelPartnerId != null)
                {
                    ...
                    if (!canAdd)
                    {
                        ...
                        skippedCount = lastRow - r;
                        break; // Stop processing when limit reached
                    }
                }
            ...
            Console.WriteLine($"DEBUG: Bulk upload completed successfully");
            
            // Create success message with details
            var successMessage = $"Bulk upload completed: {processedCount} leads processed";
            if (skippedCount > 0)
            {
                successMessage += $", {skippedCount} leads skipped due to subscription limit.";
                TempData["UpgradePrompt"] = limitMessage;
