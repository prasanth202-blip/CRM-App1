...
namespace CRM.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(ILogger<HomeController> logger, AppDbContext context, INotificationService notificationService, IWebHostEnvironment env)
        {
            _logger = logger;
            _context = context;
...
#pragma warning restore CS8629
        ...
        public async Task<IActionResult> SendContactEmail([FromBody] ContactFormModel model)
        {
            try
            {
                ...
                var emailSettings = HttpContext.RequestServices.GetRequiredService<IConfiguration>().GetSection("EmailSettings");
        ...
        public IActionResult DebugClaims()
        {
            ...
            public IActionResult GetSalesOverviewData()
            {
                ...
                if (paidCount == 0 && pendingCount == 0 && overdueCount == 0)
                {
                    ...
                    // Also provide sample recent bookings
                    if (!recentBookings.Any())
                    {
                        recentBookings = new[]
                        {
                            new { BookingId = 1001, BookingAmount = 250000m, Status = "Paid", CreatedOn = DateTime.Now.AddDays(-2).ToString("MMM dd, yyyy") },
                            new { BookingId = 1002, BookingAmount = 180000m, Status = "Pending", CreatedOn = DateTime.Now.AddDays(-5).ToString("MMM dd, yyyy") },
                            new { BookingId = 1003, BookingAmount = 320000m, Status = "Paid", CreatedOn = DateTime.Now.AddDays(-7).ToString("MMM dd, yyyy") },
                            new { BookingId = 1004, BookingAmount = 150000m, Status = "Overdue", CreatedOn = DateTime.Now.AddDays(-10).ToString("MMM dd, yyyy") },
                            new { BookingId = 1005, BookingAmount = 280000m, Status = "Pending", CreatedOn = DateTime.Now.AddDays(-12).ToString("MMM dd, yyyy") }
                        }.ToList();
                    }
                }
